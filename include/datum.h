...
extern const std::map<ComplexDataType, std::pair<RealDataType, RealDataType>> cmplx_to_real_table;

extern const std::map<RealDataType, RealDataType> cmplx_counterparts_table;

extern const std::vector<std::pair<std::string, RealDataType>> data_type_conversion;

extern const std::vector<std::pair<std::string, ComplexDataType>> complex_data_type_conversion;
...
extern const std::map<SurveyMethod, std::set<RealDataType>> method_data_types_table;
...
struct Datum
{
    ...
    static RealDataType convert_string_to_type(const std::string &string_code)
    {
        ...
        catch(std::invalid_argument &e)
        {
            for(const auto &d: data_type_conversion)
                if(Datum::istrcompare(string_code, d.first))
                    return d.second;
   ...
   */
    ...
    {
        for(const auto &t: cmplx_to_real_table)
        {
   ...
   */
    ...
    static bool istrcompare(const std::string& a, const std::string& b)
    {
